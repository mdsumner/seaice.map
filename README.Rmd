...
sf::gdal_utils("buildvrt", f, tf1 <- file.path("/vsimem", tempfile(fileext = ".vrt")), options = "-separate")
sf::gdal_utils("translate", tf1, tf2 <- file.path("/vsimem", tempfile(fileext = ".vrt")), options = c("-outsize", "10%", "10%", "-scale", "-ot", "Byte"))
sf::gdal_utils("nearblack", tf2, tf3 <- file.path("/vsimem", tempfile(fileext = ".tif")))
imrgb <- rast(tf3)
plotRGB(imrgb)
